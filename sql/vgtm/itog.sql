...
( /* остановка ПЛАН */
  ...
  gr.is_manual = 1 and
  gr.workover_type_id not in (60) and
...
( /* остановка ФАКТ */
  ...
  /*sum( case
    when (gr.fact_moveout_date >= to_date('##.##.####',  'dd.mm.yyyy') or gr.fact_moveout_date is null)
   then (gr.before_oil_rate_f * (to_date('##.##.####',  'dd.mm.yyyy') - trunc(gr.fact_stop_date,'dd')+1))
   else (gr.before_oil_rate_f * (trunc(gr.fact_moveout_date,'dd') - trunc(gr.fact_stop_date,'dd') +1))
 end) as n3*/
