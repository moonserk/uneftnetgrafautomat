...
( /* ввод в эксплуатацию ПЛАН */
  ...
  sum((to_date(to_date('##.##.####',  'dd.mm.yyyy'))-to_date(gr.plan_moveout_date)+1)*gr.after_oil_rate_p) as t4
  from gtm_netgraph_full_v gr
  where
  gr.calc_month = trunc(to_date('##.##.####',  'dd.mm.yyyy'),'mm') and
  gr.is_actual = 3 and
...
( /* ввод в эксплуатацию ФАКТ */
 ...
 sum((to_date(to_date('##.##.####',  'dd.mm.yyyy'))-to_date(gr.fact_moveout_date)+1)*gr.after_oil_rate_f) as w4
  from gtm_netgraph_full_v gr
  where
  gr.calc_month = trunc(to_date('##.##.####',  'dd.mm.yyyy'),'mm') and
  gr.is_actual = 3 and
